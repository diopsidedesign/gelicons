...
Gel.mint('gel-bar', {
   ...
   button.gel-button  {
            cursor: pointer;
            color: var(--tl-dark-or-light);
            fill: var(--tl-dark-or-light);
            stroke-opacity: 0;
            width: 100%;
            padding: 0; margin: 0;
            height: 100%;
   ...
   onload(...args) { 
      const names = iconBank.getNames(this, 'buttons')  
      this.shadowRoot.replaceChildren( Gel.el('template', {
         contents: [].concat(names).map( name => 
            iconBank.makeButton(camelCaseToDashSep(name) ))  
      }).content )
   },
   attrObservers: {
      buttons: function(newVal, oldVal) {
         this.onload(newVal,oldVal);
      }      
   },
   attrProps: ['buttons'],
